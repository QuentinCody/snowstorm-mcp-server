...
export function registerGetSchema(server: McpServer, env?: SchemaEnv) {
    const handler = createGetSchemaHandler("SNOWSTORM_DATA_DO", "snowstorm");

    server.registerTool(
        "snowstorm_get_schema",
        {
            title: "Get Staged Data Schema",
            description:
                "Get schema information for staged SNOMED CT data. Shows table structures and row counts. " +
